...
class Hog_descriptor():
    ...
    def __init__(self, img, cell_size=8, bin_size=9):
        ...
        :param bin_size: 直方图的bin个数 梯度方向量化bins
        '''
        '''
        采用Gamma校正法对输入图像进行颜色空间的标准化（归一化），目的是调节图像的对比度，降低图像局部
        的阴影和光照变化所造成的影响，同时可以抑制噪音。采用的gamma值为0.5。 f(I)=I^γ
        '''
        # 注意：这里不对图像进行平滑处理是有好处的，再不做平滑处理的时候，人像检测效果最佳，原因可能是：HOG特征是基于边缘的，平滑会降低边缘信息的对比度，从而减少图像中的有用信息。
        self.img = img
    ...
    # -----------------------------------------------------------------这里使用的是sobel边缘检测算子，可以更换其他检测算子，对比一下——————————————————————————————————————
    '''
这是文献中提到的一维算子，文献中提到，一维算子效果最好，文献地址：https://hal.inria.fr/file/index/docid/548512/filename/hog_cvpr2005.pdf
# 将图像转换为灰度图
...
hx = np.array([[-1, 0, 1]])
hy = hx.T
# 使用 cv2.filter2D 函数计算图像在水平方向上的梯度
gradscalx = cv2.filter2D(gray_img, cv2.CV_64F, hx)

# 使用 cv2.filter2D 函数计算图像在垂直方向上的梯度
gradscaly = cv2.filter2D(gray_img, cv2.CV_64F, hy)

    '''
    # 下面是使用的一维[-1, 0, 1]算子，效果明显比5*5的效果好很多
    def global_gradient(self):
        #gradient_values_x = cv2.Sobel(self.img, ddepth=cv2.CV_64F, dx=1, dy=0, ksize=5) # x方向的一阶导数
        #gradient_values_y = cv2.Sobel(self.img, ddepth=cv2.CV_64F, dx=0, dy=1, ksize=5) # Y方向的一阶导数
        hx = np.array([[-1, 0, 1]])
        hy = hx.T
        gradient_values_x = cv2.filter2D(self.img, cv2.CV_64F, hx)
        gradient_values_y = cv2.filter2D(self.img, cv2.CV_64F, hy)
        # 将直角坐标映射为极坐标，输出为向量值的大小，和向量方向
        gradient_magnitude, gradient_angle = cv2.cartToPolar(gradient_values_x, gradient_values_y, angleInDegrees=True)
        # 若向量角度在180°~360°之间，减去180度，使其在0~180之间
...
if __name__ == '__main__':
    ori_img = cv2.imread('images/test/luoding.jpg', cv2.IMREAD_GRAYSCALE)  # 灰度图
    ...
    plt.figure(figsize=(10.0, 2.0 *3.2))
    plt.subplot(1, 2, 1)
